...
| **Hugging Face** | Yes — Inference API free tier | `huggingface/...` (model-dependent) | 30 | Not capped | 32K | Medium | Supplementary |
| **Jina AI** | Yes — 1M tokens/month free | `jina_ai/jina-embeddings-v3` | 60 | Not capped | 8K | Fast | Embedding |
| **Cohere** | Yes — trial key, 20 RPM | `cohere/embed-english-v3.0`, `cohere/command-r` | 20 | Not capped | 128K | Medium | Supplementary |
| **GitHub Models** | Yes — Azure AI free tier | `github/gpt-4o`, `github/meta-llama-3.3-70b-instruct` | 15 | ~150 | 128K | Medium | Supplementary |
| **Cloudflare Workers AI** | Yes — 10,000 neurons/day | `cloudflare/@cf/meta/llama-3.3-70b-instruct-fp8-fast` | 60 | Not capped | 32K | Fast | Supplementary |
...
└── services/
    └── llm/
        ...
        │   └── (google, openrouter, cerebras, groq, together, sambanova, mistral)
        ...
        ├── key_manager.py            # Per-provider API key rotation (12 providers)
...
```yaml
model_list:
  # ── Tier 1 — Primary (Google AI Studio — 4 keys = 4× daily quota) ───────────
  - model_name: gemini-flash
    litellm_params:
      model: gemini/gemini-2.5-flash
      api_key: os.environ/GOOGLE_AI_STUDIO_API_KEY_1

  - model_name: gemini-flash-key2
    litellm_params:
      model: gemini/gemini-2.5-flash
      api_key: os.environ/GOOGLE_AI_STUDIO_API_KEY_2

  - model_name: gemini-flash-key3
    litellm_params:
      model: gemini/gemini-2.5-flash
      api_key: os.environ/GOOGLE_AI_STUDIO_API_KEY_3

  - model_name: gemini-flash-key4
    litellm_params:
      model: gemini/gemini-2.5-flash
      api_key: os.environ/GOOGLE_AI_STUDIO_API_KEY_4
  ...
  # Groq — 5 keys extending 14,400 RPD quota ───────────────────────────────────
  - model_name: groq-llama33
    litellm_params:
      model: groq/llama-3.3-70b-versatile
      api_key: os.environ/GROQ_API_KEY_1

  - model_name: groq-llama33-key2
    litellm_params:
      model: groq/llama-3.3-70b-versatile
      api_key: os.environ/GROQ_API_KEY_2

  - model_name: groq-llama33-key3
    litellm_params:
      model: groq/llama-3.3-70b-versatile
      api_key: os.environ/GROQ_API_KEY_3

  - model_name: groq-llama33-key4
    litellm_params:
      model: groq/llama-3.3-70b-versatile
      api_key: os.environ/GROQ_API_KEY_4

  - model_name: groq-llama33-key5
    litellm_params:
      model: groq/llama-3.3-70b-versatile
      api_key: os.environ/GROQ_API_KEY_5
  ...
  - model_name: huggingface-embed
    litellm_params:
      model: huggingface/sentence-transformers/all-MiniLM-L6-v2
      api_key: os.environ/HUGGINGFACE_API_KEY_1
...
router_settings:
  ...
  retry_after: 5
  fallbacks:
    - gemini-flash: [gemini-flash-key2, gemini-flash-key3, gemini-flash-key4, gemini-flash-lite, deepseek-v3, qwen3-235b, groq-llama33, groq-llama8b]
    - deepseek-v3: [deepseek-v3-key2, qwen3-235b, groq-llama33, groq-llama8b]
...
### 20.3 Detailed Explanation (Domain-Specific AI Routers)
EduScribe AI implements an intelligent **AI Router** architecture that splits incoming tasks among specialized routers, each with its own domain-specific fallback chain:
...
```
        ...
        │               │               │
        ▼               ▼               ▼
    LLM Router     Embedding Router   Vision Router
        │               │               │
        ▼               ▼               ▼
 Groq            Jina           Gemini
...
Instead of a single linear chain, the fallback sequence is optimized for the specific task domain:

**1. Note Generation**
Gemini (Primary) → Groq → OpenRouter → Cloudflare

**2. Coding**
Groq [Qwen] → OpenRouter → GitHub Models

**3. Mathematics**
Groq [DeepSeek R1] → Gemini → OpenRouter

**4. Embeddings**
Jina (Primary) → Hugging Face → Cohere → Cloudflare

**5. OCR / Vision**
Gemini (Primary) → Cloudflare → OpenRouter

**6. Speech-to-Text**
Groq Whisper (Primary) → Cloudflare Whisper

Whenever the current provider fails due to any of the following conditions — API quota exceeded, timeout, rate limiting, server errors, or temporary maintenance — the request is automatically retried using the next available provider in this domain's sequence.

### 20.4 Internal Workflow (Note Generation Example)

```mermaid
flowchart LR
    A[Gemini 2.5 Flash] -->|fails after retries| B[Groq Llama 3.3]
    B -->|fails after retries| C[OpenRouter DeepSeek/Qwen]
    C -->|fails after retries| D[Cloudflare Llama]
```

**Rationale:** The fallback sequence is ordered to prioritize the provider identified as strongest for the specific domain (e.g. Gemini for notes, Groq DeepSeek-R1 for math, Jina for embeddings). This ensures that the pipeline attempts the highest-quality option first and only degrades to alternative models when genuinely necessary.
...
Each provider may have one or more associated API keys. Instead of always using the same key until it is entirely exhausted, EduScribe AI uses a **round-robin rotation** approach to avoid hitting rate limits and distribute load evenly across all available free-tier accounts.

The documented example for Groq (5 keys) illustrates this behavior:
...
The exact same round-robin strategy is applied to Gemini, OpenRouter, Cohere, Hugging Face, and Cloudflare keys. Only once every available key for a given provider has reached its quota limit does the system escalate to switching providers entirely, at which point the Fallback Strategy (Chapter 20) takes over.

### 21.4 Internal Workflow

```mermaid
flowchart TD
    A[Incoming Request for Provider] --> B[Retrieve next key in round-robin list]
    B --> C{Key Quota/Rate Limit available?}
    C -->|Yes| D[Use Key]
    C -->|No, all keys exhausted| E[Switch Provider via Fallback Strategy]
```

**Rationale:** Round-robin API Key Rotation exists as a layer beneath provider-level fallback because distributing load helps prevent rate limits from being tripped in the first place. Furthermore, switching providers is a more disruptive operation than switching keys within the same provider — a key switch preserves the currently selected model's quality characteristics. Exhausting all available keys before escalating therefore maximizes the amount of work completed using the preferred primary provider.
