...
class LLMManager:
    ...
    def _provider_from_model(self, model: str) -> str:
        ...
        if model_lower.startswith(
                "cloudflare/") or model_lower.startswith("@cf/"):
            return "cloudflare"
    ...
    async def generate(
        ...
        **litellm_kwargs: Any,
        ...
        response_format = litellm_kwargs.get(
            "response_format", {}).get(
            "type", "text")
        context.capabilities = CapabilityDetector.detect(
            messages, expected_output_format=response_format)
        ...
        if override_model:
            ...
            starting_model, starting_provider = self._select_starting_model(
                config)

        try:
            prompt_tokens = litellm.token_counter(
                model=starting_model, messages=messages)
            context.estimated_tokens = prompt_tokens
            context.min_context_window = prompt_tokens + context.max_tokens
        except Exception as e:
            logger.debug(
                f"token_counter failed, defaulting context requirement to 0: {e}")
        ...
        if cached_response:
            logger.info(
                f"LLM_CACHE_HIT | req_id={context.request_id} | task={task.value}")
        ...
        async def _call_provider(provider: str, model: str) -> dict[str, Any]:
            ...
            try:
                ...
                MetricsRecorder.record(
                    context, provider, model, latency, usage)
                ...
                logger.warning(
                    f"LLM_REQUEST_FAILED | req_id={context.request_id} | provider={provider} | reason=ResponseParseError")
