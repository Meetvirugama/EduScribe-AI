...
class EmbeddingManager:
    ...
    def __init__(
        ...
        self.fallback_chain = [
            ...
            {"provider": "cloudflare", "model": "cloudflare/@cf/baai/bge-large-en-v1.5"}
        ]
        ...
        **litellm_kwargs: Any,
        ...
        for entry in self.fallback_chain:
            ...
            if not self.quota_tracker.has_quota(provider):
                logger.info(f"Skipping {provider} embedding due to quota exhaustion.")
                continue
            ...
            try:
                ...
                logger.warning(f"Embedding transient failure on {provider}, trying next fallback. Error: {exc}")
                last_error = exc
