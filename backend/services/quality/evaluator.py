...
class HallucinationGuard:
    ...
    def score(self, text: str, transcript_text: str, concepts: List[Any]) -> float:
        ...
        novel = {n for n in notes_proper if n not in transcript_lower and n not in concept_names}
...
class QualityEvaluator:
    ...
    def evaluate_artifact(
        ...
        schema_valid = self._schema_validator.validate(artifact_type, artifact_data)
        
        # Convert artifact to string for text-based checks
        artifact_text = str(artifact_data)
        
        hallucination_risk = self._hallucination.score(artifact_text, transcript_text, concepts or [])
        ...
        if hallucination_risk > 0.3:
            warnings.append(f"Potential hallucination detected (risk: {hallucination_risk:.0%})")
