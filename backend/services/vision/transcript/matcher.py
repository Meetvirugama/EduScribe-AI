...
TRANSCRIPT_CONTEXT_WINDOW: int = getattr(
    settings, "TRANSCRIPT_CONTEXT_WINDOW", 1)
...
class TranscriptMatcherService:
    ...
    def _get_or_create_index(self, transcript_path: str,
                             video_id: str) -> TranscriptIndex:
        ...
        try:
            ...
            logger.error(
                "Failed to load transcript from %s: %s",
                transcript_path,
                exc)
            segments = []
    ...
    def score_similarity(self, ocr_text: str, transcript_text: str) -> float:
        ...
        # Token set ratio handles subsets (slide bullets inside spoken
        # paragraphs)
    ...
    def match_frames_to_transcript(
        ...
        if not os.path.exists(transcript_path):
            logger.warning(
                "Transcript file not found: %s – using 0 similarity.",
                transcript_path)
            return [{
                **f,
                "transcript_text": "",
        ...
        for frame in frames:
            ...
            candidates = index.get_context_segments(
                ts_ms, window=TRANSCRIPT_CONTEXT_WINDOW)
        ...
        logger.debug(
            "Matched %d frames to transcript segments using O(log N) index.",
            len(enriched))
        return enriched
