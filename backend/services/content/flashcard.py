...
class FlashcardGenerator(BaseContentService):
    async def generate_flashcards(self, context: LectureContext) -> Dict[str, Any]:
        ...
        if not learning_note.strip():
            logger.warning("FlashcardGenerator: detailed_notes_md is empty. Falling back to transcript.")
