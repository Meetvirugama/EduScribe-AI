...
class ExporterService:
    ...
    def export_all(transcript: CanonicalTranscript) -> Dict[str, str]:
        ...
        # 2. Timestamped TXT
        ts_txt_path = os.path.join(export_dir,
                                   f"{transcript.video_id}_timestamped.txt")
        ...
        with open(srt_path, "w", encoding="utf-8") as f:
            for seg in transcript.segments:
                ...
                f.write(
                    f"{ExporterService._format_time_srt(seg.start)} --> {ExporterService._format_time_srt(seg.end)}\n")
                f.write(f"{seg.text}\n\n")
        paths["srt"] = srt_path
        ...
        with open(vtt_path, "w", encoding="utf-8") as f:
            ...
            for seg in transcript.segments:
                f.write(
                    f"{ExporterService._format_time_vtt(seg.start)} --> {ExporterService._format_time_vtt(seg.end)}\n")
                f.write(f"{seg.text}\n\n")
        paths["vtt"] = vtt_path
