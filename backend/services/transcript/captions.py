...
class CaptionService:
    ...
    @staticmethod
    def discover_and_acquire(video_id: str, requested_language: str = "en") -> Dict[str, Any]:
        """
        ...
        try:
            selected_track = transcript_list.find_manually_created_transcript([requested_language])
            source_type = "manual"
        except Exception:
            # 2. Try automatic caption
            try:
                selected_track = transcript_list.find_generated_transcript([requested_language])
                ...
                try:
                    ...
                    for manual_track in transcript_list._manually_created_transcripts.values():
                        if manual_track.is_translatable:
                            selected_track = manual_track.translate(requested_language)
